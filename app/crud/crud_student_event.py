...
class CRUDStudentEvent(CRUDBase[StudentEvent, StudentEventCreate, StudentEventUpdate]):
    ...
    def update_student_event(
        ...
        """
        Update an existing event for a student.
        """
    ...
    def remove_student_eevent(self, db: Session, *, event_id: str) -> StudentEvent:
        """
        Delete an event by its ID.
        """
        obj = db.query(self.model).filter(self.model.event_id == event_id).first()
        if obj:
            db.delete(obj)
            db.commit()
        return obj
